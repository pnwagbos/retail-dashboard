...
function displayDataGuide() {
  const requiredColumns = {
    "OrderDate": "Date of the order in DD/MM/YYYY format",
  ...
  const requiredList = document.getElementById("requiredColumnsList");
  requiredList.innerHTML = Object.entries(requiredColumns)
    .map(([key, desc]) => `<li data-tooltip="${desc}">${key}</li>`).join("");

  const kpiList = document.getElementById("kpiGuideList");
  kpiList.innerHTML = Object.entries(kpis)
    .map(([key, desc]) => `<li data-tooltip="${desc}">${key}</li>`).join("");
}

function toggleSidebar() {
  document.getElementById("dataGuidePanel").classList.toggle("closed");
}
...
function renderTables(results) {
  ...
  document.getElementById("reorderTable").innerHTML = `
    ...
    ${results.lowStock.map(row => `<tr><td>${row.Product}</td><td>${formatNumber(row.StockLevel)}</td><td>${formatNumber(row.ReorderPoint)}</td></tr>`).join("")}
  `;
}
