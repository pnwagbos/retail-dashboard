...
function displayDataGuide() {
  const requiredColumns = ["OrderDate", "Cost", "ReorderPoint", "StockLevel", "Product", "Revenue"];
  const kpis = ["Total Sales", "Total Profit", "AOV", "Profit Margin", "Top Products", "Low Stock Alerts"];

  const requiredTable = document.getElementById("requiredColumnsTable");
  requiredTable.innerHTML = requiredColumns.map(col => `<tr><td>${col}</td></tr>`).join("");

  const kpiTable = document.getElementById("kpiGuideTable");
  kpiTable.innerHTML = kpis.map(kpi => `<tr><td>${kpi}</td></tr>`).join("");
}
